...
def test_cloud_cron_execution(set_k8s_auth, controller_scenario):
    ...
    cj = v1_batch.read_namespaced_cron_job(
        name=cronjob_name, namespace="pve-cloud-controller"
    )
    ...
    while True:
        ...
        pods = v1.list_namespaced_pod(
            namespace="pve-cloud-controller", label_selector=f"job-name={job_name}"
        ).items
...
def test_delete_ingress(
    ...
    resp = api.create_namespaced_ingress(namespace="default", body=ingress_body)
...
def test_update_ingress(
    ...
    created = api.create_namespaced_ingress(namespace="default", body=ingress_body)
    ...
    updated = api.replace_namespaced_ingress(
        name="test-update-ingress", namespace="default", body=created
    )
...
def test_ingress_connectivity(get_test_env, get_k8s_api_v1, deployments_scenario):
    ...
    assert (
        x509_cert.issuer.get_attributes_for_oid(NameOID.COMMON_NAME)[0].value
        == "nginx-ca"
    )
...
def test_external_ingress_dns(
    get_k8s_api_v1, get_proxmoxer, get_test_env, deployments_scenario, get_moto_client
):
